// ML Insights Generation Utilities
import type { 
  Project, 
  Proposal, 
  TimeEstimate, 
...
export function generateMachineLearningInsights(
  projects: Project[],
  proposals: Proposal[],
  ...
  const workloadPredictions = generateWorkloadPredictions(timeEstimates, projects, professionalRoles);
  const revenueForecasts = generateRevenueForecasts(projects, proposals, timeEstimates, period);
  const clientInsights = generateClientInsights(timeEstimates, clientCompanies, projects);
  const serviceEfficiencyInsights = generateServiceEfficiencyInsights(timeEstimates, services);
  const seasonalPatterns = detectSeasonalPatterns(timeEstimates, projects, period);
  const staffingRecommendations = generateStaffingRecommendations(timeEstimates, projects, professionalRoles);
  const abnormalPatterns = detectAbnormalPatterns(timeEstimates, projects, clientCompanies);
  const profitabilityPredictions = generateProfitabilityPredictions(projects, timeEstimates, clientCompanies);
...
function generateWorkloadPredictions(
  timeEstimates: TimeEstimate[],
  projects: Project[],
  professionalRoles: ProfessionalRole[]
) {
  // Group time estimates by role
  const roleWorkloads = new Map<number, { 
    roleId: number,
    roleName: string, 
    currentHours: number,
  ...
  professionalRoles.forEach(role => {
    ...
    const availableCapacity = tierMap[tier];
...
function generateRevenueForecasts(
  ...
  for (let i = 0; i < 6; i++) {
    ...
    periods.push({
      label: formatPeriodLabel(periodDate, period),
      date: periodDate
    });
  }
  ...
  const confirmedRevenue = periods.map(period => {
    const periodRevenue = calculateRevenueInPeriod(projects, timeEstimates, period.date, period);
    return {
      period: period.label,
  ...
  const projectedRevenue = periods.map(period => {
    ...
    proposals.forEach(proposal => {
      ...
      if (!proposal.estimatedStartDate || isInPeriod(proposal.estimatedStartDate, period.date, period)) {
  ...
  const totalRevenueForecast = periods.map((period, i) => {
    return {
      period: period.label,
...
function generateClientInsights(
  ...
  const clientInsights = clientCompanies.map(client => {
    ...
    const totalHours = clientTimeEstimates.reduce((sum, te) => sum + Number(te.hours || 0), 0);
    const totalRevenue = clientTimeEstimates.reduce((sum, te) => {
      const hourlyRate = Number(te.rate || 0);
...
function generateServiceEfficiencyInsights(
  timeEstimates: TimeEstimate[],
  services: Service[]
) {
  const serviceEfficiency = services.map(service => {
    ...
    const totalHours = serviceTimeEstimates.reduce((sum, te) => sum + Number(te.hours || 0), 0);
...
function detectSeasonalPatterns(
  ...
  const monthlyWorkload = seasonalPatterns.map(pattern => {
    const date = new Date();
    date.setMonth(pattern.month);
    
    return {
      period: formatPeriodLabel(date, 'month'),
  ...
  const peakPeriods = monthlyWorkload
    .filter(m => m.factor > 1.3)
    .map(m => m.period);
  
  // Identify slow periods
  const slowPeriods = monthlyWorkload
    .filter(m => m.factor < 0.8)
    .map(m => m.period);
  
  return {
    seasonalWorkload: monthlyWorkload,
    peakPeriods,
    slowPeriods,
...
function generateStaffingRecommendations(
  ...
  professionalRoles.forEach(role => {
    ...
    const currentHours = roleTimeEstimates.reduce((sum, te) => sum + Number(te.hours || 0), 0);
    ...
    const staffCount = role.staffCount || 2; // Default to 2 if not specified
    const hoursPerStaff = role.tier === 'top' ? 160 : (role.tier === 'mid' ? 180 : 200);
...
function detectAbnormalPatterns(
  timeEstimates: TimeEstimate[],
  projects: Project[],
  clientCompanies: ClientCompany[]
) {
  ...
  projects.forEach(project => {
    ...
    const actualHours = projectTimeEstimates.reduce((sum, te) => sum + Number(te.hours || 0), 0);
  ...
  timeEstimates.forEach(te => {
    ...
    const rate = Number(te.rate);
    // Anomalous if rate is very low or very high
    if ((rate < 50 || rate > 300) && Number(te.hours || 0) > 5) {
...
function generateProfitabilityPredictions(
  projects: Project[],
  timeEstimates: TimeEstimate[],
  clientCompanies: ClientCompany[]
) {
  ...
  const clientProfitability = clientCompanies.map(client => {
    ...
    return {
      ...
      predictedProfitMargin: Math.round(futureProfitMargin * 100) / 100,
  ...
  const projectProfitability = projects.map(project => {
    ...
    const totalHours = projectTimeEstimates.reduce((sum, te) => sum + Number(te.hours || 0), 0);
...
function isInPeriod(date: Date, periodDate: Date, periodConfig: { label: string, date: Date }): boolean {
...
function calculateRevenueInPeriod(
  projects: Project[], 
  timeEstimates: TimeEstimate[],
  periodDate: Date,
